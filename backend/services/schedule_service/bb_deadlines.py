...
async def fetch_blackboard(cas_account: str, cas_password: str) -> list[Deadline]:
    ...
    try:
        async with _blackboard_authenticated_session(cas_account, cas_password) as (client, headers, tab_url, default_tab_url, response):
            ...
            seed_upload_urls = set(_extract_upload_assignment_urls(response.text, base_url))
            ...
            async def fetch_one(url: str) -> Deadline | None:
                try:
                    ...
                    logger.warning("bb.fetch: request failed url=%s err=%s", url, type(exc).__name__)
                    return None

                if response.status_code >= 400:
                    if response.status_code == 500 and _looks_like_transient_bb_500(response):
                ...
                try:
                    return _parse_deadline_from_upload_assignment_html(response.text, url)
            ...
            deadlines_raw = await asyncio.gather(*(fetch_one(url) for url in upload_urls))
            deadlines = [
                deadline
                for deadline in deadlines_raw
                if deadline and deadline.type == DeadlineType.ASSIGNMENT and deadline.due_at and deadline.due_at >= now_local
            ]
            ...
            if not deadlines and upload_urls:
                ...
                deadlines_raw2 = await asyncio.gather(*(fetch_one(url) for url in sorted(upload_url_set2)))
                deadlines = [
                    deadline
