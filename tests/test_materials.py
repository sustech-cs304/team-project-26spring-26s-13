...
async def test_upload_material_success(async_client: AsyncClient, auth_headers: dict):
    ...
    with patch(
        ...
        resp = await async_client.post(
            ...
            files={"file": ("lecture.pdf", io.BytesIO(b"%PDF fake content"), "application/pdf")},
        )
...
async def test_upload_material_unsupported_type(async_client: AsyncClient, auth_headers: dict):
    ...
    with patch(
        ...
        resp = await async_client.post(
            "/api/materials/upload",
            headers=auth_headers,
            files={"file": ("notes.exe", io.BytesIO(b"MZ binary"), "application/octet-stream")},
        )
